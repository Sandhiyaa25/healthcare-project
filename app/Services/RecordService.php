...
class RecordService
{
    ...
    private const ENCRYPTED_FIELDS = ['chief_complaint', 'diagnosis', 'treatment', 'notes'];
    ...
    public function archive(int $id, int $tenantId, int $userId, string $role, string $ip, string $userAgent): void
    {
        if (!in_array($role, ['admin', 'doctor'])) {
            throw new ValidationException('Only admin or doctor can archive medical records');
        }
        ...
        if (!empty($record['archived_at'])) {
            throw new ValidationException('Medical record is already archived');
        }

        $this->recordModel->archive($id);
